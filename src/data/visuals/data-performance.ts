...
export const dataVisuals: Record<string, VisualSpec> = {
  caching: {
    width: 760,
    height: 320,
    caption: 'A hit returns from memory in about 1 ms. A miss pays the database round trip, then fills the cache.',
    nodes: [
      { id: 'user', kind: 'client', label: 'User', x: 40, y: 120, w: 140, h: 74 },
      { id: 'api', kind: 'server', label: 'API', x: 230, y: 120, w: 140, h: 74 },
      { id: 'cache', kind: 'cache', label: 'Redis', sub: 'hit 1 ms', x: 430, y: 30, w: 160, h: 96, stat: ['Hit rate', '91%'] },
      { id: 'db', kind: 'sql', label: 'PostgreSQL', sub: 'miss 50 ms', x: 430, y: 200, w: 160, h: 82 },
    ],
    edges: [
  ...
  'cache-strategies': {
    ...
    steps: [
      { from: 'client', to: 'app', label: 'Read: caller asks for item' },
      { from: 'app', to: 'cache', label: 'Cache-aside: check cache, miss', outcome: 'warning' },
      { from: 'app', to: 'db', label: 'App loads it from database' },
      { from: 'app', to: 'cache', label: 'App stores it with TTL' },
      { from: 'client', to: 'app', label: 'Write: caller updates item' },
      { from: 'app', to: 'cache', label: 'Write-behind: cache takes it' },
      { from: 'app', to: 'client', label: 'Acknowledged before the database' },
      { from: 'cache', to: 'db', label: 'Flushed to the database later' },
    ],
  },
  ...
  redis: {
    width: 760,
    height: 300,
    caption: 'An in-memory cache with a memory limit, TTLs and an async replica. The database stays the source of truth.',
    nodes: [
      { id: 'api', kind: 'server', label: 'API servers', sub: 'cache-aside', x: 40, y: 110, w: 170, h: 80 },
      { id: 'redis', kind: 'cache', label: 'Redis', sub: 'maxmemory, allkeys-lru', x: 290, y: 20, w: 190, h: 96, stat: ['Round trip', '< 1 ms'] },
      { id: 'db', kind: 'sql', label: 'Database', sub: 'source of truth', x: 290, y: 200, w: 190, h: 80 },
      { id: 'replica', kind: 'cache', label: 'Redis replica', sub: 'async copy', x: 560, y: 28, w: 170, h: 80 },
    ],
    edges: [
      { from: 'api', to: 'redis', tone: 'ok', rate: 4, outcome: 'cache-hit' },
      { from: 'api', to: 'db', tone: 'violet', rate: 0.6, label: 'on miss' },
      { from: 'redis', to: 'replica', tone: 'muted', dashed: true, rate: 1 },
    ],
    steps: [
      { from: 'api', to: 'redis', label: 'GET product:42, in RAM' },
      { from: 'redis', to: 'api', label: 'Hit: answered under 1 ms', outcome: 'cache-hit' },
      { from: 'api', to: 'redis', label: 'GET product:99: miss', outcome: 'warning' },
      { from: 'api', to: 'db', label: 'App reads the database' },
      { from: 'api', to: 'redis', label: 'SET with TTL; LRU key evicted' },
      { from: 'redis', to: 'replica', label: 'Copied to replica, asynchronously' },
    ],
  },
  ...
  'database-indexing': {
    width: 760,
    height: 300,
    caption: 'The database reads pages. A full scan reads all 50,000; the B-tree index reads one per level, then the row.',
    nodes: [
      { id: 'app', kind: 'server', label: 'App', sub: 'WHERE email = ?', x: 20, y: 110, w: 150, h: 80 },
      { id: 'planner', kind: 'sql', label: 'Query planner', sub: 'picks scan or index', x: 220, y: 110, w: 180, h: 80 },
      { id: 'index', kind: 'search', label: 'Index on email', sub: 'B-tree, 3 levels', x: 480, y: 15, w: 250, h: 96, stat: ['Pages read', '3 + 1 row'] },
      { id: 'table', kind: 'storage', label: 'users table', sub: '5M rows in 50,000 pages', x: 480, y: 190, w: 250, h: 96, stat: ['Full scan', '~1.2 s'], alert: true },
    ],
    edges: [
      { from: 'app', to: 'planner', tone: 'brand', rate: 2.4 },
      { from: 'planner', to: 'table', tone: 'warn', rate: 0.6, outcome: 'warning' },
      { from: 'planner', to: 'index', tone: 'ok', rate: 1.8 },
      { from: 'index', to: 'table', tone: 'ok', rate: 1.8 },
    ],
    steps: [
      { from: 'app', to: 'planner', label: 'Find one user by email' },
      { from: 'planner', to: 'table', label: 'No index: read 50,000 pages', outcome: 'warning' },
      { from: 'table', to: 'planner', label: 'Found after about 1.2 s', outcome: 'warning' },
      { from: 'planner', to: 'index', label: 'With index: walk 3 levels' },
      { from: 'index', to: 'table', label: 'Pointer: fetch 1 table page' },
      { from: 'table', to: 'planner', label: '4 pages read, 0.3 ms' },
      { from: 'planner', to: 'app', label: 'Row returned to the app' },
    ],
  },
