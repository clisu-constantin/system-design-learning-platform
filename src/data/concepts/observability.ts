    ...
    quiz: [
      {
        ...
        options: [
          'Switch every service to DEBUG so each failing request leaves ten times more lines describing what it did',
          'Log structured fields with a trace_id every service passes on, so one query returns a whole request',
          'Add nodes to the log cluster so a full-text grep across all 6 services returns in seconds, not minutes',
          'Write the failures to a separate errors file per service',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing - the payments error line holds the cause, and the cause is all an investigation needs',
          'The trace_id itself, because only INFO lines carry it, so the query stops matching anything',
          'The context: the route, that orders called payments, and the 502 the user saw',
          'The error message itself, because ERROR lines are sampled at a lower rate than INFO',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing, since a uniform 10% sample keeps every signal in the same proportion as before',
          'Each kept line must carry a sampling weight field, so every line grows and most of the saving disappears again',
          'The sampler rewrites headers, so the trace id stops being propagated to downstream services',
          'Requests survive in fragments and 90% of errors are dropped; sample per trace id and keep all errors',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Redact the token and card number at the source, before the line is written',
          'Keep it - logs are internal, only engineers can read them, and the detail saves hours in the next incident',
          'Keep it but lower the level to DEBUG, so it is only written while someone is actively debugging',
      ...
      },
      {
        ...
        options: [
          'Keep parsing logs, but move them to a columnar store so the p95 query runs faster',
          'Emit a latency histogram metric per route and keep logs for explaining single requests',
          'Have each instance log its own p95 once a minute as one line, and graph those lines',
          'Stop graphing latency per route and only check it during incidents',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The worker needs its own log store, because queue consumers write to a different index than the API',
          'Workers run outside a request, so they cannot log a trace id at all',
          'The queue is too slow, so the trace id expires before the worker picks up the message',
      ...
      },
      {
        ...
        options: [
          'A wrong password is expected and handled; at ERROR it buries the errors a human must act on',
          'None - every failed login is security relevant, so ERROR is the right level to make sure it is kept',
      ...
      },
      {
        ...
        options: [
          'Log one summary line per chunk with counts, plus a line per failed item',
          'Move the logs to a faster disk so writing a million lines no longer slows the job',
          'Switch the lines to JSON so they compress better and take less room on disk',
          'Log each item twice, once to disk and once to the network, so none is lost',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Payments were fixed by the release; the complaints are about older orders',
          'The log store lost data during the release; add replicas',
          'The message was reworded; query a stable field such as event = "payment.failed"',
          'The new release turned on error sampling; switch sampling off for payments',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Keep one week searchable and delete the rest - most searches touch recent days',
          'Keep a year in the search cluster',
          'Tier retention: recent days searchable, older logs archived in cheap object storage',
          'Log less, at WARN and above only, so a full year fits in the current cluster',
        ],
        answer: 2,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'The average over a 1-hour window, so short noise does not hide the real trend',
          'CPU per host, since slow pages usually mean one machine is saturated',
          'p99 from the latency histogram - the average hides the slow tail',
          'The request count, to see whether traffic rose when the complaints began',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          '1.8% of requests are slow: over 1 in 100, so p99 lands in them; under 5 in 100, so p95 does not',
          'p95 is recomputed every 5 minutes and p99 every 10 seconds, so p95 has not caught up with the spike yet',
          'The histogram has no bucket above 1 s, so p95 cannot see the 2 s queries',
      ...
      },
      {
        ...
        options: [
          'Nothing notable - labels are stored as strings on each sample, so they cost a few bytes each',
          'The histogram becomes more precise, because each user gets their own buckets',
          'Only dashboards get slower, since queries must group by 25,000 values',
          'Series multiply by about 25,000 - millions the backend must store and index',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Yes - a counter only drops when requests fail, so traffic was lost',
          'No - counters restart at zero with the process; rate() handles the reset',
          'Yes, the metrics backend lost the data it had scraped from the old pods',
      ...
      },
      {
        ...
        options: [
          'Nothing - with twenty pods behind a balanced load balancer, the average of their p99s is the service p99',
          'They should take the maximum of the twenty, so the slowest pod sets the number',
          'Quantiles cannot be averaged; sum histogram buckets across pods, then compute p99',
          'They should use p95, which is stable enough to be averaged',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The spike was not real - it was shorter than a scrape',
          'The metrics backend drops samples shorter than the window to save storage',
      ...
      },
      {
        ...
        options: [
          ...
          'The full URL with the query string, so every request can be told apart',
          'Nothing, remove all labels',
          'The path in route, and the order id moved to a separate order_id label',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'None - three buckets are enough, since the percentile is interpolated between 100 ms and 500 ms',
          'No bound at 300 ms, so the share under it is a guess; add a 300 ms bucket',
          'Histograms cannot measure SLOs; a summary with a precomputed p95 is needed',
      ...
      },
      {
        ...
        options: [
          ...
          'A longer retention period, so the total shows the trend over weeks',
          'CPU per host, since a failing route usually burns CPU on retries',
          'A larger y-axis',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'CPU, memory and GC time per host - the resources every problem shows up in',
          'Log line count per level, so a jump in ERROR lines shows up at once',
          'Request rate, error ratio and a latency histogram per route (RED)',
          'One gauge per internal function, so any slow step can be spotted early',
        ],
        answer: 2,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'Add CPU to the gateway, since the gateway graph is where the extra latency shows up',
          'Compare the average latency of the 6 services and pick the one that grew the most',
          'Open traces of slow checkouts and find the span with the most self time',
      ...
      },
      {
        ...
        options: [
          'Parent spans include their children; order-service is waiting, payment owns the time',
          'order-service got slower too - its bar grew just as much, so it needs its own fix',
          'The trace is broken - a parent bar cannot be longer than the 35 ms of work it did itself',
      ...
      },
      {
        ...
        options: [
          'Kafka dropped the message after the publish span, so the notification service never ran',
          'The consumer found no traceparent and started a new trace nothing links to',
          'The notification service stopped recording spans once the header option was off',
      ...
      },
      {
        ...
        options: [
          'The card-risk service needs a higher sampling rate, since its spans are sampled apart from the caller',
          'The clocks of the two services drifted apart, so card-risk spans fall outside the payment span',
          'The card-risk service is too fast to be traced',
          'The new client sends no traceparent header, so card-risk starts a new trace',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'All 1,000, because the SDK marks a trace as sampled when an error happens in it',
          'About 100',
          'About 10 - the coin is flipped before the outcome is known',
          'None, because head-based sampling drops error traces on purpose to save space',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Each order becomes its own operation name; use GET /orders/{id} and put the id in an attribute',
          'Span names cannot contain digits, so the exporter rejects the span and the trace loses its root',
          'The trace_id stops being propagated',
          'Nothing - more detail in the name makes the trace of one order easy to find later',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'In the span name, such as "checkout user-8812", so it shows in the waterfall',
          'As a user_id label on the request latency metric, so you can filter by customer',
      ...
      },
      {
        ...
        options: [
          'A faster database for the pricing service',
      ...
      },
      {
        ...
        options: [
          'In the database, because it is the only dependency and the queries must be slower than they look',
          'In the network between the services',
          'Inside the report service - waiting for a thread or connection, or work before the first query',
          'Nowhere - the trace is missing spans, so its timings cannot be trusted',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Yes - a child span must end before its root span, or the waterfall total and the response time disagree',
          'Yes - work that runs after the response should have been recorded as a separate trace',
          'No, but the response time now includes the email, since it is part of the same trace',
          'No - the consumer runs after the response; the trace covers it, the response time is the root span',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Search payment-service logs in the minute around the failure',
          'Query the log store for that trace_id - every line of the request carries it',
      ...
      },
      {
        ...
        options: [
          'Writing spans to log files instead, which any tracing vendor can import',
          'Using head-based sampling everywhere, so every vendor gets the same traces',
          'Instrumenting with OpenTelemetry and exporting through a collector',
          'Running both vendors in parallel from the start, so switching is just turning one off',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The database driver in payment-service records it as a client span',
          'Postgres reads the trace_id from the SQL text',
          'The collector matches queries to traces by their timestamps and the payment-service host name',
    ...
    quiz: [
      {
        ...
        options: [
          ...
          'An outside probe loading the site, plus an alert on traffic below its expected range',
          'A lower error-ratio threshold, such as 0.5%, so even a small rise in failures pages at once',
      ...
      },
      {
        ...
        options: [
          ...
          'Two separate problems: the gateway fails 4% on its own, and Payments fails 10% on top of it',
          'The gateway is the cause, because every request passes through it first',
      ...
      },
      {
        ...
        options: [
          'Requests per second, since more requests means more connections are needed',
          'p50 latency',
          'CPU on the application servers, the usual sign that a service is full',
      ...
      },
      {
        ...
        options: [
          ...
          'A disk usage graph per host, since a full disk often slows every request',
          'Deploy markers - a line on every graph where a release happened',
          'Auto-refresh every second, so the rise shows the moment it starts',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'One overview of the golden signals per service, with drill-downs behind it',
          'A raw log search filtered to ERROR across all 30 hosts, sorted by newest first',
          'A list of every alert that fired in the last day, grouped by host and severity',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Black-box, because a probe sees what users see',
          'Neither - it is not a problem until users complain',
          'Log search, because the pool logs a warning each time it grows',
          'White-box: it sees internal state, so you can act before users feel it',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Every alert rule fires at once, so all of on-call is paged within the first minute',
      ...
      },
      {
        ...
        options: [
          'Good news - less load, and 0% errors means nothing is failing',
          'The error ratio proves everything is fine',
          'Something in front of the service stops requests (DNS, CDN, a client release)',
          'The metrics backend is overloaded and dropping samples',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'A new dashboard with a panel for Android 4.2 users in Germany with large carts',
          'Wide events or traces with many attributes, sliced by version, country and cart size',
          'A higher-resolution CPU graph',
          'A latency metric with a user_id label, so any group of users can be filtered out later',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Probes cannot detect outages that only affect some regions of the internet',
          'The probe interval of 5 minutes is too long to catch a regional failure',
          'The probe checks the page but should check CPU and memory on each host',
          'It skips the path users take; probe from outside, in several regions',
        ],
        answer: 3,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'Delete the rule and rely on customers to report failures',
          'Raise the threshold to 20%, well above the level the blips reach',
          'Add a for duration of about 2 minutes to the rule',
          'Page a secondary on-call as well, so blips get a second pair of eyes',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The page comes about 2 minutes late, while users already hurt',
          'None - for only filters blips, so a lasting fault pages the moment it starts',
          'The rule never fires for lasting faults',
          'The rule fires twice, once at the start and again after 2 minutes',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'It pages at once: the 10% payment failure is above the 5% threshold',
          'It never pages: the gateway sees 4%, so users hurt and nothing fires',
          'It pages after the for duration, once 10% has held long enough',
      ...
      },
      {
        ...
        options: [
          'Keep it - CPU at 70% is the early warning that comes before users feel anything',
          'Raise it to 95%',
          'Stop paging on it; keep CPU on the dashboard and page on user symptoms',
      ...
      },
      {
        ...
        options: [
          'Too few people on call - 200 alerts a week is more than one engineer can handle',
      ...
      },
      {
        ...
        options: [
          'Ticket: at that rate the budget lasts exactly 30 days',
          'Page, because budget is being spent every hour and the SLO is at risk',
          'Neither - at exactly 1x the SLO is met, so there is nothing to do',
          'Page, and roll back the last deploy before more budget is spent',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Page on-call now, so the renewal starts before anything breaks',
          'No alert - someone will notice the renewal email in time',
          'Email the whole company so every team knows the expiry date',
      ...
      },
      {
        ...
        options: [
          'Longer for durations on every service',
          'Removing the database alert',
          'Grouping and inhibition: one database page, with downstream alerts muted',
      ...
      },
      {
        ...
        options: [
          ...
          'Send it to a different team that might know the component better',
          'Lower its threshold so it fires more often and the team learns what it means over time',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The threshold is too high; lower it to 0.5% so even a small rise in errors pages',
          'The for duration is too long, so the rule has not had time to fire yet',
          'The rule is broken',
          'No requests arrive, so nothing counts as an error; alert on traffic and probes',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'A ticket when the trend says "disk full in under 4 days"',
          'Page at 70% instead, so there are days left to react before it fills',
          'No alert - disks are cheap, so add capacity once a quarter',
          'Page at 99% instead, so nobody is woken while there is still room',
        ],
        answer: 0,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'Users are exaggerating - a 99.98% SLI means only 2 in every 10,000 checkouts failed this month',
          'The failing requests never reach the server - the load balancer or network drops them',
          'The server needs more CPU, since slow responses make the mobile app time out',
          'The SLI window is too long, so a short outage is averaged away over the month',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Stop counting bots and probes, which pad the valid count with easy successes',
          'Measure at the load balancer instead of the API server, so dropped requests count',
      ...
      },
      {
        ...
        options: [
          'About 94%, because the health checks pad the count with easy successes',
          'Exactly 90% - health checks are separate endpoints, so they do not change the ratio',
      ...
      },
      {
        ...
        options: [
          'The average latency over the month, compared with a 300 ms target',
          'The maximum latency seen that day',
          'The p50 latency, because it is more stable than p99 from minute to minute',
      ...
      },
      {
        ...
        options: [
          ...
          'Not as bad events - the service did what it promised; document that rule',
      ...
      },
      {
        ...
        options: [
          ...
          'Availability of the API behind the dashboard',
      ...
      },
      {
        ...
        options: [
          'Nothing - every service meets its target, so the journey must be fine',
          'Add a seventh service that watches the other six and reports their health',
          'Raise every service to 99.99%, so the six together fail far less often',
          'Add a journey SLI: checkouts completed over checkouts started',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Use the higher number - the app also counts failures from bad mobile networks',
          'Use the lower number and discard the other, since it is the safer one',
          'Agree what counts as good, what as valid, and where it is measured',
          'Average the two numbers, since both teams measure honestly and neither is wrong',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Keep all 15 - every extra SLI is one more way a problem cannot slip through unmeasured',
          'Keep two or three that users feel, and keep the rest as debugging metrics',
          'Keep only CPU, since it predicts latency and errors for every service',
          'Replace them all with one uptime check that pages when the service is down',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Shorten the month to two weeks so outages leave the report faster',
          'Exclude the last day of each month, when most deploys happen',
          'Report only the best week of each month to smooth out outliers',
        ],
        answer: 0,
        explanation:
...
budget spent         -> freeze risky changes, fix reliability first`,
    ...
    quiz: [
      ...
      },
      {
        ...
        options: [
          'Ship it - an SLO is only a goal, and the budget resets in 10 days anyway',
          'Ship it, then raise the SLO next month to make up for the lost minutes',
          'Hold it, or cut its risk first with a flag, canary and tested rollback',
      ...
      },
      {
        ...
        options: [
          ...
          'About 24 minutes, and a page - the burn rate is 100x',
          'All 43 minutes, and a ticket, since the budget is gone before anyone acts',
          'None, because 90% of requests still succeed and the SLO counts requests',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          '0.2x - nothing',
          '20x - page immediately, because 0.2% is twenty times 0.01%',
          '2x - a ticket, since the budget lasts about 15 days',
          '2x - page immediately, before the budget is spent',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'No - over the hour the burn is about 8x, under 14.4x',
          'Yes - 100% errors is always a page',
          'Yes - the 5-minute window reads 1000x, far above 14.4x, and one window is enough',
      ...
      },
      {
        ...
        options: [
          ...
          'Fewer failures users do not notice, for half the budget plus a second region',
          'It doubles the error budget, from 43 minutes to about 86 a month',
          'It gains a contract with customers, who can then claim credits whenever it is missed',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Nothing changes - the team keeps working as before and treats 100% as a stretch goal',
          'The budget is zero: every failure breaks the SLO, so it stops guiding decisions',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Raise the SLO to 99.999% to match what the service really delivers, so callers know what to expect',
          'Keep expectations at 99.9%, for example by spending spare budget on planned outages',
      ...
      },
      {
        ...
        options: [
          'The budget shrinks to 4.3 minutes and the burn becomes 5x: gone in about 6 days',
      ...
      },
      {
        ...
        options: [
          ...
          'An SLO per journey: tighter for checkout, looser for help pages',
          'Drop the SLO for checkout',
          'Give the help pages the tighter SLO, because they get more traffic and more users see them',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Yes - two weeks above the allowed 0.1% error rate should have paged someone by now',
          'Yes - burn-rate alerts cannot see slow leaks',
          'No - it should never alert on this',
          'No - 1.5x is a slow leak: a ticket, since the budget lasts about 20 days',
        ],
        answer: 3,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'None - the numbers agree, so engineers and customers read one figure and never argue over it',
          'The SLA is too loose, so customers get no real guarantee',
          'Customers cannot measure 99.9%',
          'No buffer: missing the internal target breaches the contract at the same moment',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'About 43 minutes',
          'About 3 hours 36 minutes',
          'About 7 hours 12 minutes',
          'About 21 minutes 36 seconds',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'No credit - missing the internal SLO triggers your budget policy, not a payment',
          '10% of the monthly fee, because 99.80% is below the 99.9% target that was promised',
          'A refund of the full month',
          'A 5% credit, since 99.80% sits between the SLO and the SLA',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'None - 99.95% is lower than 99.9%, so the providers already cover it',
          'The three together allow about 99.7%, so 99.95% needs independent fallbacks',
          'Providers usually beat their SLAs by a wide margin, so it is safe in practice',
          'The SLA must equal the lowest of the three provider SLAs, which is 99.9% here',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Sue for the rest, since the SLA guarantees compensation for real losses',
          'Choose a provider whose SLA pays a larger credit percentage, such as 50% of the fee',
          'Nothing, 500 euro is fair',
          'An SLA is a signal, not insurance - protect the critical path in the architecture',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Breaches are settled once a year, in a single credit on the invoice after the contract anniversary',
          'The provider forgot',
          'Credits are usually not automatic: you must file a claim with evidence before a deadline',
          'Credits are paid out in cash by bank transfer, not deducted on invoices',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'No - that SLA counts only errors, so slowness is not a breach',
      ...
      },
      {
        ...
        options: [
          'Both, with the same terms, so free users can trust the product enough to upgrade',
          'Only the free tier, since it has by far the most users and the most complaints',
          'Neither - SLAs are only offered by large cloud providers, never by startups',
          'The enterprise plan; the free tier gets an internal SLO without a contract',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing - the SLA is only paperwork for the sales team',
          'The error budget grows, since the contract now covers the stricter number',
          'Credits come due before your own error budget is spent',
          'The SLI becomes more accurate, because it is checked against a stricter number',
        ],
        answer: 2,
        explanation:
