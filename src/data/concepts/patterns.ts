    ...
    quiz: [
      {
        ...
        options: [
          'Fan-out on read, because copying every post into up to 5,000 timelines wastes storage and writes',
          'Fan-out on write: up to 5,000 writes per post, so each read is one lookup',
          'A hybrid with a 10,000 follower threshold, so the design is ready before large accounts appear',
          'No fan-out: one global post list, filtered per user',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The timeline cache is full, so each new post evicts entries the workers then have to rewrite',
          'Feed reads compete with the workers for the same database and cut the 300,000 writes a second',
          'Each post is about 167 s of writes, but a new one arrives every 120 s',
          'Listing 50M followers from the follow graph takes longer than the 2 minutes between posts',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The post costs 0 writes; a read is the timeline plus one pull, 2 queries',
          'The post still costs 50M writes, just spread over a longer time',
          'The post costs 10,000 writes, one per follower up to the threshold, and the rest pull it at read time',
          'Nothing changes for this account, because the hybrid rule only applies to followers who join later',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The number of followers those power users have, since each follower adds work to their feed',
          'The 2,000 accounts they follow, all queried and merged on every read',
          'The size of the posts table as a whole',
          'The rate at which the power users post, since each post triggers a timeline rebuild',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The CDN still serves the post image, so the purge request for that URL was missed',
          'The post row was soft-deleted, so feed queries still return it until a hard delete runs',
          'The delete must fan out too, out of all 1 million timelines',
          'The client cached the old feed locally and needs a pull-to-refresh to fetch the new one',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The follow graph is eventually consistent; the posts appear once the new follow edge replicates',
          'Posts are pushed only at post time; backfill recent ones on follow',
          'The 800-entry timeline cap dropped them; raise the cap',
          'Fan-out on write cannot support new follows at all; move the whole feed to fan-out on read',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Call the 4 services in sequence to avoid overloading them',
          'Add 3 retries on every branch so a slow or failed call gets another chance to succeed',
          'Time out each branch; render without recommendations if late',
          'Nothing: every service is 99.9 percent available, so the page is 99.9 percent available too',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Each read is 1 lookup plus 30 pulls; cache the hot celebrity posts',
          'Each of the 30 accounts was pushed into the timeline 30 times, so the timeline is too long to read',
          'The hybrid falls back to pure fan-out on read for this user; lower the threshold to 1,000',
          'The follow graph caps celebrity follows at 20 per user',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Switch every user to fan-out on read, so nothing is copied into timelines at all',
          'Compress the post text inside each timeline entry, which cuts memory by about 4x',
          'Move the timelines from memory to disk, where 20 million of them fit cheaply',
          'Cap each timeline at its newest 800 entries; query older pages',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Fan-out on read: store each reading once, build a dashboard when opened',
          'A hybrid with a 10,000 follower threshold, the standard setup for any feed-like system',
          'Fan-out on write, with the prebuilt dashboards cached at a CDN',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The ordinary posts wait in one queue behind 30 million celebrity writes',
          'The celebrity post saturates the network, so every other post has to wait for bandwidth',
          'The feed service is overloaded by millions of celebrity followers opening the app at once',
          'Timelines are locked while the celebrity post is written',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'It buys nothing: fan-out on read also has to copy the post to every follower first',
          'Instant visibility, paid for by a full merge on every read',
          'It buys faster reads, because nothing is precomputed, and costs slower writes',
    ...
    quiz: [
      {
        ...
        options: [
          'Raise the queue limit from 10,000 to 100,000 so the backlog has room to drain',
          'Add consumers, and bound the queue so excess is rejected or shed',
          'Retry failed messages faster',
          'Cut the visibility timeout from 30 s to 5 s so stuck messages return sooner',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Never - 1,000 slots absorb the surplus while the workers drain',
          'After about 3.3 s, then all 300 per second are refused',
      ...
      },
      {
        ...
        options: [
          'Rejections stop; depth and wait grow without limit',
          'Nothing visible - the workers catch up once the burst of 300 msg/sec passes',
          'The producer is throttled to 80 msg/sec, the rate the workers can take',
      ...
      },
      {
        ...
        options: [
          'Block the application until the collector catches up, so no sample is lost',
          'Spill samples to local disk with no size limit and replay them later',
          'Return 429 to the end user',
          'Drop samples when the buffer is full',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Drop them silently and let the client notice the missing confirmation',
          'Reject with 503 or 429 and a Retry-After header',
      ...
      },
      {
        ...
        options: ['Only 1 call', 'Up to 9 calls (3 x 3)', 'Up to 16 calls', 'Up to 4 calls'],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The window shrinks to zero; the sender waits for the reader',
          'The receiver drops the extra packets, and the sender slows down when it sees the losses',
          'The kernel keeps growing the receive buffer so no data is ever dropped',
      ...
      },
      {
        ...
        options: [
          'Nothing - every request is still processed, just 30 s later than usual',
          'The workers need more memory to hold the queue',
          'Raise the client timeout to 60 s so every queued request can still be answered',
          'Drop work past its deadline; reject when the wait exceeds 10 s',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Queue depth above 100,000, twice the normal level',
          'Oldest message older than 2 minutes',
          'Producer publish error rate above 1%',
          'Producer CPU above 80% as publishes pile up',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Health checks - they are not real traffic and cost nothing to skip',
          'Checkouts - they are the heaviest requests, so shedding them frees the most',
          'An equal share of every kind of request',
          'Bulk exports, so checkouts and health checks keep capacity',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The queue grows past its bound until the consumer frees the lock',
          'Deadlock - each side waits for the other',
          'The consumer times out on the lock and skips that item',
    ...
    quiz: [
      {
        ...
        options: [
          'Nothing - checkout never calls recommendations, so its requests are unaffected',
          'Checkout slows by a few milliseconds because the CPU is busier',
          'Within about 2 s every thread waits on recommendations; checkout fails',
          'Checkout keeps working for 30 s, then fails when the first calls time out',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Turn on bulkheads so checkout keeps its own threads',
          'Raise the call timeout to 10 s so fewer recommendation calls fail',
          'Add more checkout load so it wins a bigger share of the threads',
          'Restart Payments',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'About 2 calls/s answered after 5 s; the rest fail fast to the fallback',
          'All 20 calls per second are answered, each about 5 seconds later',
      ...
      },
      {
        ...
        options: [
          'Never - with a 1 s cap no call holds a thread long enough to exhaust the pool',
          'Only when the dependency returns errors quickly',
          'When the load is high: 300 calls/s x 1 s still wants 300 threads',
          'Only when the timeout is longer than 30 seconds, as defaults often are',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'It protects the service better, because no call is lost while it waits for a slot',
          'Nothing different - both settings behave the same',
          'It makes the recommendations service recover faster, since fewer calls hit it at once',
          'Waiting callers hold their own threads, moving the pile-up upstream',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The semaphore times out the call after a few seconds and frees the thread',
          'At most 25 caller threads hang, and they hang forever',
      ...
      },
      {
        ...
        options: [
          'The reports fill the shared connection pool; every database feature waits',
          'Nothing - each dependency already has its own thread pool, so reports are contained',
          'The database refuses the report queries once it hits its own connection limit',
      ...
      },
      {
        ...
        options: [
          'A bigger shared pool for everyone, so the batch job no longer fills it',
          'A circuit breaker on the database that opens when timeouts spike',
          'A per-tenant concurrency limit, or cells for groups of tenants',
      ...
      },
      {
        ...
        options: [
          ...
          'The cost of the pattern: reserved threads sit idle on purpose',
      ...
      },
      {
        ...
        options: [
          'All three do the same job of stopping a cascade, so one of them is enough',
          'The bulkhead retries, the timeout isolates, the breaker queues',
          'Timeout caps how long, bulkhead how many, breaker whether to call',
    ...
    quiz: [
      ...
      },
      {
        ...
        options: [
          ...
          'The charge really happened; explain the refund, do not hide it',
          'Switching to choreography removes the charge line',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Use a 60 s timeout so a slow reply is never mistaken for a lost one',
          'Switch to choreography, which delivers exactly once',
          'Send the saga id as an idempotency key that Payment checks',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Park it in an alerting queue for a human to finish',
      ...
      },
      {
        ...
        options: [
          'Nothing - orchestration only renames the events and moves them to one topic',
          'It removes the need for compensations, because the orchestrator can roll back',
          'One place for the sequence, at the cost of an always-on coordinator',
      ...
      },
      {
        ...
        options: [
          ...
          'An event Loyalty reacts to is enough; an orchestrator is overhead',
      ...
      },
      {
        ...
        options: [
          'The saga forgot to take a database lock on the stock rows for the whole saga',
          'Inventory should have been the last step, after payment succeeded',
          'The release ran in the wrong order',
          'Sagas have no isolation: the reservation was visible to others',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Kafka and the provider cannot prepare, and a coordinator crash blocks all',
          'Two-phase commit is not atomic, so a crash mid-commit would lose orders',
          'Two-phase commit only works across more than three participants',
          'None: two-phase commit is the standard way to join services',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Its state was persisted per step, so it resumes by sending shipping',
      ...
      },
      {
        ...
        options: [
          ...
          'No component owns the saga, and no event reached Order',
          'Choreography does not support compensations, so none was sent',
      ...
      },
      {
        ...
        options: [
          'Compensate every step, including a return request for the parcel and a refund',
          'Retry the invoice until it succeeds; the saga can only go forward',
    ...
    quiz: [
      {
        ...
        options: [
          ...
          'The order exists, and no event was ever published or retried',
      ...
      },
      {
        ...
        options: [
          ...
          'It was sent anyway; the broker is outside the transaction',
          'Kafka holds it until the transaction commits',
          'It was never sent, because nothing leaves the service before COMMIT',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'No order and no event - both inserts rolled back together',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'The rows are still unsent, so the three events go out again',
      ...
      },
      {
        ...
        options: [
          'Record each event id with its effect, in one transaction; skip seen ids',
          'Set Relay crash chance to 0 in production',
          'Switch back to Commit, publish, which never sends duplicates',
          'Delete each outbox row before publishing it, so it goes out once',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Publish from the request path again - faster and just as safe',
          'Nothing can be done, the outbox is always 5 s behind',
          'Remove the outbox table and make consumers poll the orders table',
          'Poll more often (more database queries) or use CDC (a new pipeline)',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Both read the same rows; claim them with FOR UPDATE SKIP LOCKED',
      ...
      },
      {
        ...
        options: [
          'Key each message by its order id',
          'Use one partition for the whole topic so everything is in order',
      ...
      },
      {
        ...
        options: [
          'Add more relay instances to split the polling work',
          'Move the outbox to a separate database',
          'Delete or archive sent rows on a schedule',
          'Poll less often, so the slow query runs fewer times',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Two-phase commit cannot guarantee atomicity across two systems',
          'Postgres cannot take part in two-phase commit, so it cannot even start',
          'It would publish every event twice, once per phase',
          'Most brokers cannot join it, and a coordinator crash blocks both',
        ],
        answer: 3,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'Some followers end at 10 and some at 12, depending on arrival order',
          'A write conflict that an operator must resolve by hand',
          'The same value everywhere, in the order the leader chose',
          'Both values, kept as siblings for the app to merge',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Add three more followers to spread the write load',
          'Route some writes to the three followers as well',
          'Promote a follower to a second leader to double write capacity',
          'Cut and batch writes, then shard with one leader each',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Retry them with backoff; the gap is expected',
          'Show a fatal error and discard the input, since the data may be corrupt',
          'Send them to a follower, which can accept writes until promotion',
      ...
      },
      {
        ...
        options: [
          'Any of them - they replay the same log, so they are identical',
          'The one at 1000, missing the fewest writes',
          'The one at 990, because it is the most stable',
          'The one with the lowest load, so it can take the writes',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Synchronous replication, so every write is on all nodes',
          'A bigger heap, so the garbage collection pause never happens',
          'Fencing: writes with the old term are rejected',
      ...
      },
      {
        ...
        options: [
          'The single node stays leader and the two elect a second one',
          'The two-node majority elects a leader; the old one must stop writing',
      ...
      },
      {
        ...
        options: [
          'On the leader, which serves every read and write',
          'Nowhere - reads are free',
          'It is refused once the followers hit their limit',
          'On the three followers, not the leader',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Write conflicts, and a rule to merge or pick',
          'Nothing - multi-leader is strictly faster',
          'Slower reads in both regions, since every read must check both leaders',
          'Loss of all followers, since each leader replaces them',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'The read is always refused until all 3 replicas agree',
          'A stale read; choose W + R > N, for example W = 2, R = 2',
          'The write is lost; choose W = 0 so writes never wait',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Make every follower a leader so each one has the latest data',
          'Send that user to the leader briefly after a write',
          'Remove the followers',
          'Restart the followers so they catch up with the leader',
        ],
        answer: 1,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          '2 - 80/sec is close enough to the 100/sec average',
          '3 - 120/sec keeps up with 20/sec to spare',
          '100 - one consumer per item per second',
          '1 consumer with a buffer big enough for a whole day',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing visible - the 500-message buffer absorbs the gap',
          'The buffer stays empty but the workers look busier',
          'Depth grows 20/sec; the 500 buffer fills in 25 s, then rejects',
          'Depth grows by 80/sec, the output of the two workers',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'One consumer per partition, so 12 of the 16 sit idle',
          'Kafka limits every consumer group to 4 consumers by default',
          'The 12 new consumers need a warm-up period before they fetch',
      ...
      },
      {
        ...
        options: [
          ...
          'Add more consumers so each item waits less time',
          'Make the buffer bigger',
          'Route each account to one consumer by account id',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Producers block for about 10 s of every burst',
      ...
      },
      {
        ...
        options: [
          'Immediately - producers get errors as soon as the gap appears',
          'Late: when items are hours old or memory runs out',
          'Never - a 5% gap is within normal tolerance',
          'After exactly one buffer size of items',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'It is redelivered to another consumer after the visibility timeout',
          'The producer sends it again',
          'It is lost - acknowledge only after the work is done',
      ...
      },
      {
        ...
        options: [
          'Doubling the consumers to process twice as fast',
          'Giving each consumer more CPU',
          'A bigger buffer',
          'Batching many items per API call',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'No - the idle time is the headroom for the next burst',
      ...
      },
      {
        ...
        options: [
          '200 - the buffer lets the consumers keep up',
          '160 - the average of the two',
          '120; the other 80/sec fill the buffer, then are refused',
          '0 - the pipeline stalls once the buffer overflows',
        ],
        answer: 2,
        explanation:
...
Every synchronous hop is a shared fate decision.`,
    ...
    quiz: [
      ...
      },
      {
        ...
        options: [
          'About 250 ms - the sum of all three',
          'About 83 ms - the average of the three',
          'About 120 ms - the slowest call',
          'About 50 ms - the fastest call',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Only 9 of 12/sec are served; waits grow to the 2 s timeout',
          'The API rejects 3 requests per second at once with 429',
          'Workers speed up to 4 per second to meet the load',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing - the workers are still too slow, so users still time out',
          'Every request now succeeds at once and all the work is done',
          'The workers get faster',
          'Fast answers, but the queue grows 3 per second',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Its threads run out, and even unrelated requests stop',
          'Nothing - the operating system cancels the calls after a while',
          'Only the requests that call the dependency slow down',
          'The load balancer removes the dependency automatically',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Refuse at once: 300 ms cannot fit in the 100 ms left',
      ...
      },
      {
        ...
        options: [
          'Request/response - the user needs the answer to continue',
          'A queue - it is always more reliable',
          'A queue, with the result emailed to the user',
          'Either - the user does not notice a few ms',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Raise every timeout on the path to 5 minutes, above the 2 minute build',
          '202 Accepted plus a status URL; a worker builds it',
          'Generate the report twice in parallel to make it faster',
      ...
      },
      {
        ...
        options: [
          ...
          'Only network bandwidth for the unsent result',
          'Nothing, because closed pages do not count as traffic',
          'Wasted capacity; stop the work when the client disconnects',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The same in both designs, since B is down either way',
          'Synchronously orders fail for 30 s; with the event they succeed',
          'With the event, orders fail for 30 s until B consumes again',
      ...
      },
      {
        ...
        options: [
          '2 s for B and 2 s for C, the usual client default',
          'No timeouts - the budget is enforced by the user',
          '300 ms each, leaving 200 ms for the work of A',
          '800 ms each, the full budget',
        ],
        answer: 2,
        explanation:
