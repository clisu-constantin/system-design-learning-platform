    ...
    quiz: [
      {
        ...
        options: [
          'The queue stays near empty, because five workers pull in parallel and keep up',
          'Depth grows by 100 msg/s, and so does the wait for each new message',
          'Producers are throttled to 400 msg/s, because the broker applies backpressure when workers lag',
          'Messages beyond the 400 msg/s the workers can take are dropped',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The message is lost, because the broker already handed it to a worker, so the customer is never charged',
          'The broker sees the charge in the payment provider and deletes the message, so nothing repeats',
          'It is redelivered after the visibility timeout; only an idempotency key on the charge stops a second charge',
          'Nothing repeats - at-least-once brokers also deduplicate, so each message is processed exactly once',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Rejections start after about 125 s instead of 12.5 s; the deficit stays',
          'The 40 msg/sec deficit disappears, because the queue can now hold everything that arrives',
          'The workers get faster, because they have more to pull',
          'Rejections start sooner, because a queue of 5,000 is slower to append to than one of 500',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing - the order service still fails, because the queue has to deliver to the email service too',
          'Orders succeed; the emails wait in the queue and go out once the provider recovers',
          'Orders succeed, but the emails from those 20 minutes are lost, because no worker could send them',
      ...
      },
      {
        ...
        options: [
          'A, because its depth is 20 times higher and depth is what a backlog alert watches',
          'Neither - both queues are far below a 50,000 message depth limit, so nothing is at risk',
      ...
      },
      {
        ...
        options: [
          'Nothing - a queue is first in, first out, so five workers pulling from it also finish in that order',
          'Add more workers, so each event finishes before the next one of that order is pulled',
          'Route by order id to a partition or FIFO group, so one consumer applies order 42 in turn',
          'Make the producer wait 500 ms between the three events',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'It is redelivered forever and blocks its ordered group; cap attempts and dead-letter it with an alert',
          'The broker deletes it after the first failed attempt, so it cannot block anything and needs no fix',
          'Raise the visibility timeout, so the worker has more time to finish the message before it is retried',
          'Add workers, so the failing message takes a smaller share of the pool and the rest flows',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing unusual - the broker sees the worker is still connected and waits for the job',
          'Every job fails at 30 s with a timeout error, and the worker drops its half-done work',
          'The broker slows delivery to one message every 40 s to match the job length',
          'After 30 s the message is visible again and a second worker starts the same job',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The unfinished part of the batch is redelivered to another worker when the crash is detected',
          'Those messages are lost - acking before processing is at-most-once delivery',
          'Every message in the batch is processed twice, once by the crashed worker and once by another',
          'The broker rolls the batch back automatically',
        ],
        answer: 1,
        explanation:
          'Once acknowledged, the broker forgets the message, so work that crashed is gone. That is at-most-once: fast, and lossy. At-least-once acknowledges after the work instead and pays with possible duplicates. At-most-once is acceptable for sampled metrics, not for orders.',
      },
      {
        id: 'mq-10',
        prompt:
        ...
        options: [
          'Rejects the whole burst, because 300 msg/sec is above the 120 msg/sec the workers can do',
          'Grows to about 360 during the burst, then drains in about 18 s',
          'Nothing - the burst goes straight to the workers',
          'Grows to about 400 and stays there, because the workers only just keep up with 100 msg/sec',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Memory use on the workers, since waiting jobs hold open connections',
          'The number of producers',
          'Backlog per worker, or the age of the oldest message',
          'Network bytes in and out, because the jobs are IO-bound',
        ],
        answer: 2,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'All 8 share the load evenly, so throughput rises by a third over 6 consumers',
          ...
          'The group is rejected, because a group cannot have more members than partitions',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Key each event by order id, so one order always lands on one partition',
          'Use a single partition for the whole topic, since that is the only way Kafka keeps order',
          'Use a random key so the load spreads evenly',
          'Rely on Kafka ordering the whole topic by the timestamp of each record',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Deploy the fix and reset the search group offset to three days ago',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'They are deleted by retention before it reads them',
          'Kafka pauses the producers until the slow group catches up, the same as a full queue',
      ...
      },
      {
        ...
        options: [
          ...
          'Key by something with many more values, such as user id',
          'Turn on log compaction so the hot partition holds fewer records',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Run the partition reassignment tool with the new count of 12',
          'Partitions cannot be removed: create a 12-partition topic and migrate to it',
          'Delete 36 partitions; Kafka moves their records to the remaining 12',
          'Set the partition count to 12 in the topic config; keys keep their old partitions',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Kafka skips the record, because it was already delivered to a member that crashed',
      ...
      },
      {
        ...
        options: [
          'It is a separate consumer group with its own offsets',
          'The projector reads a copy of the partitions that Kafka makes for each group',
          'The projector has priority on the broker',
          'Billing and the projector take turns, and billing skips its turn when it is busy',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'A self-run Kafka cluster with one partition, so jobs stay in order',
          'A Kafka topic with a compacted log',
          'A managed queue such as SQS, with per-message ack and retry built in',
      ...
      },
      {
        ...
        options: [
          ...
          'The last 7 days, by starting a new group at the oldest record kept',
          'The whole month, because Kafka keeps every record a group has not read',
          'Nothing until an existing group shares its committed offsets with it',
        ],
        answer: 1,
        explanation:
 ...
 [orders q]   [audit q]`,
    ...
    quiz: [
      ...
      },
      {
        ...
        options: [
          'Direct routing is exact string matching, so order.* matches no real key',
          'A direct exchange delivers each message to only one of the bound queues',
          'Switching the exchange type deletes the existing bindings, so they must be re-created',
      ...
      },
      {
        ...
        options: [
          'Make the order service publish each event a second time, to a queue for fraud',
          'Have the fraud service consume from the payments queue too, since it holds every order',
          'Have the payments service forward each message to the fraud service',
          'Declare a fraud queue bound with order.placed; the producer does not change',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'The broker holds it until both instances acknowledge, then deletes it',
          'One of the two instances gets it - they share the work',
          'It is rejected, because a queue with two consumers cannot pick one',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Add more consumers',
          'Set a small prefetch, such as 10, so no consumer hoards unacked messages',
          'Switch the exchange to fanout, so every consumer gets the jobs',
      ...
      },
      {
        ...
        options: [
          ...
          'A larger prefetch, so the invoice is buffered on the consumer',
          'A fanout exchange',
          'A shorter message TTL, so the invoice is retried sooner',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Publisher confirms, so the producer knows each message reached the broker',
          'A dead letter exchange to catch messages lost in the restart',
          'A topic exchange instead of a direct one, which keeps its routes on disk',
      ...
      },
      {
        ...
        options: [
          'Every queue grows, because the exchange slows down to the pace of Analytics',
          'The publisher starts getting errors once the slowest queue falls behind',
          'Only the Analytics queue grows - the other subscribers keep up',
          'Analytics starts receiving only every second event, to match its pace',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Raise the prefetch, so other messages get through while this one keeps looping',
          'Reject it without requeue, to a dead letter exchange and a watched queue',
          'Delete the queue and recreate it',
          'Switch to auto-ack, so the broker drops it after the first failed delivery',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'None - its exclusive queue went away with it, so nothing was kept',
          'All of them, from its queue, which waits for it',
          'Only the last one, which the exchange retains per binding',
          'All of them, because the exchange kept them until Email reconnected',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'A higher prefetch and a 60-second sleep in the consumer',
          'A fanout exchange that copies the message to a retry service',
          'Publishing with a delay header on a direct exchange',
          'A retry queue with a 60 s TTL that dead-letters back to the main exchange',
        ],
        answer: 3,
        explanation:
...
No service knows the others exist.`,
    ...
    quiz: [
      ...
      },
      {
        ...
        options: [
          'The order service adds a call to the email service after the order is saved',
          'The order service publishes a new FirstPurchase event, since it knows the history of each customer',
          'Nothing in the order service: a new consumer of OrderPlaced checks for a first purchase',
      ...
      },
      {
        ...
        options: [
          'It is a command in disguise: the publisher decides what the subscriber must do',
          'Event names must be in capital letters',
          'Emails should never be sent from events',
          'Nothing, as long as only the email service subscribes to it',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The broker notices the missing event and publishes it on behalf of the order service',
          'The event is published twice after the restart; give each event a unique id',
          'The database write is rolled back automatically, because the publish did not happen',
          'The order is saved but no event goes out; use an outbox in the same transaction',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Exactly-once delivery from the broker, which would never send the event twice',
      ...
      },
      {
        ...
        options: [
          'Add more consumers to every service so no event waits',
          'Replace the events with synchronous calls everywhere, so every step is visible in one trace',
          'Orchestrate the ordered steps with compensation; keep independent reactions choreographed',
          'Increase the event retention so the stuck event can be replayed',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'By deleting the OrderPlaced event from the topic',
      ...
      },
      {
        ...
        options: [
          'The event was lost; publish OrderPlaced again so Order history receives it',
          'The read model lags; show the order as pending, or read it from the order service',
          'The broker is too slow and must be replaced with a faster one',
          'The order service must call Order history synchronously, so the order is there before it answers',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The broker is misconfigured and delivers out of order',
          'The publisher sent them in the wrong order, since a broker never reorders',
          'Cancellations have a higher priority, so the broker delivers them first',
          'Ordering holds only within a partition or queue, not across topics',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Change event schemas additively only, checked against a schema registry',
          'Tell every consumer before each release, so they can change at the same time',
          'Never change an event',
          'Use a fanout exchange',
        ],
        answer: 0,
        explanation:
          'With events you do not know every consumer, so removing or renaming a field is a breaking change for someone. Additive changes - add amount, keep total - and a registry check keep evolution safe. Telling every consumer is exactly the coordination event-driven design tries to avoid - and you cannot tell the ones you do not know.',
      },
      {
        id: 'eda-11',
        prompt:
        ...
        options: [
          'Reading the order service code, since it publishes every event in the flow',
          'Restarting the email service',
          'A correlation id on every event, plus tracing and per-consumer lag',
          'Publishing the event again and watching which service fails',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'No - use a fanout exchange so every service sees the payment',
          'No - the user needs the answer now; call payments',
        ],
        answer: 3,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'One, to whichever subscriber asks for work first, so no event is handled twice',
          'One, to Email, because it is listed first',
          'Three - each subscriber queue gets its own copy',
          'Three, but only after all three subscribers have confirmed the previous one',
        ],
        answer: 2,
        explanation:
          'Fanout copies every message into every bound queue, so one publish becomes three independent copies. "Whichever asks for work first" describes a work queue, where consumers compete for one message. Nobody waits for confirmations: each subscriber handles its copy on its own.',
      },
      {
        id: 'ps-2',
        prompt:
        ...
        options: [
          'Email and Analytics carry on; the CRM sync queue holds its events until it returns',
          'All three stop until CRM sync is back, so no subscriber gets ahead',
          'The publisher gets an error for each event CRM sync cannot take',
          'CRM sync events are rerouted to Email while it is down',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Redis keeps them in memory until the subscriber reconnects and asks',
          'Redis retries them for 60 seconds, which covers a 20 second restart',
          'The publisher gets an error for each subscriber that is missing and republishes',
          'They are lost for that subscriber - Redis pub/sub stores nothing',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'No - one lost message leaves an instance with wrong prices until it restarts',
          'Yes - a missed message is caught by the 60-second TTL anyway',
          'No - pub/sub cannot reach twenty subscribers at the same moment',
          'Yes - Redis pub/sub never loses messages, so the TTL is not needed',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Each service still gets its own copy, since each one is bound to the exchange',
      ...
      },
      {
        ...
        options: [
          ...
          'A dashboard panel for partition count and disk use',
          'A faster publisher, so events reach analytics sooner',
          'A second topic just for analytics, so it reads less',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Turn on exactly-once delivery in the broker, so a rebalance never resends a message',
      ...
      },
      {
        ...
        options: [
          'Yes - the publish call returns once every subscriber has handled the event',
          'Yes, if the email service subscribes before the publish',
          'Yes, with a fanout exchange that confirms each delivered copy',
          'No - publishers hear nothing back; use request-response',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          ...
          'Add a sixth subscriber that caches orders, and point the other five at its cache',
          'Publish the event five times, one copy for each subscriber, so they stop sharing',
          'Make the subscribers poll less often, say once a minute instead of on each event',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'As two consumers in the same consumer group, so both read the topic',
          'Analytics reads each record from the email service after it is handled',
          'In two separate consumer groups, each with its own offsets',
          'Through one consumer that reads once and forwards to both services',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'It adds a call to the fraud service after each capture',
          'Nothing - the fraud service subscribes and deploys on its own',
          'It publishes a copy to a second topic that only fraud reads',
      ...
      },
      {
        ...
        options: [
          ...
          'A second subscriber that reorders events by timestamp',
          'A larger retention, so late events can be reread',
          'Publish with the payment id as the key',
        ],
        answer: 3,
        explanation:
    ...
    quiz: [
      {
        ...
        options: [
          'Raise the HTTP timeout of the signup endpoint to 30 seconds, so slow emails finish',
          'Create the user, enqueue the welcome email and answer; a worker sends it with retries',
          'Send the email first, then create the user, so a failed email stops a half-made account',
      ...
      },
      {
        ...
        options: [
          'Still 20 ms - removing the queue removes a hop, and the job runs after the answer',
      ...
      },
      {
        ...
        options: [
          'It waits behind the import, maybe for hours; give transactional work its own queue',
          'It jumps the line, because it is a small job',
          'It is dropped because the queue is busy',
          'The import slows itself down automatically when other jobs arrive',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The broker must use exactly-once delivery, so the second delivery never happens',
          'Deploys must wait until the queue is empty and no job is running',
          'Nothing - a deploy drains workers cleanly, so redelivery never happens',
          'The job must be idempotent, keyed by user id and job type',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing - the broker finishes the jobs the workers had started',
          'Half-finished jobs; on SIGTERM stop pulling, finish or requeue the current job, then exit',
          'The whole queue is lost with the workers; the fix is a durable queue that survives restarts',
          'Producers stop sending jobs until the workers come back',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Sending the confirmation email, so the customer has it before the page loads',
      ...
      },
      {
        ...
        options: [
          ...
          'The HTTP error rate of the API, which rises whenever the jobs behind it fail',
      ...
      },
      {
        ...
        options: [
          'A way to learn the result: a status URL to poll, or a webhook when it is ready',
          'Nothing - 202 means the export is done and the file is ready to download',
          'A longer HTTP timeout on the export endpoint, so the client can wait for the file',
          'A bigger queue',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing - the workers borrow idle connections from the web tier pool when they need one',
          'They need 500 against 250 free, so many are refused; give workers a small pool',
          'The database quietly queues the extra connection requests and serves them as others close',
          'Workers do not use database connections',
        ],
        answer: 1,
        explanation:
          '50 x 10 = 500 on top of 150 is 650 against a limit of 400. A database refuses connections past its limit (PostgreSQL answers "too many clients"), and the web tier suffers too. Workers need their own, smaller budget, and should never hold a connection during an external call.',
      },
      {
        id: 'bw-11',
        prompt:
        ...
        options: [
          'Users wait longer for their 202, because the API now waits for a worker to free up before it enqueues',
          'User wait stays 20 ms, but the queue grows by 2 jobs/sec and jobs finish later and later',
          'Both stay the same, because 4 workers can absorb the extra 3 jobs/sec',
    ...
    quiz: [
      {
        ...
        options: [
          ...
          'Keep 30 s - duplicates are harmless, since the thumbnail is the same',
          'Set it above the slowest normal case, say 2 minutes, and let long tasks extend it',
          'Turn acknowledgements off, so the broker never redelivers a task',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Retry 10 times, in case the address fixes itself on a later attempt',
          'Send it straight to the dead-letter queue with the error',
          'Retry immediately with no delay, since backoff only wastes time here',
          'Drop the task silently, since a retry can never make it succeed',
        ],
        answer: 1,
        explanation:
      ...
      },
      {
        ...
        options: ['30%', '9%', '2.7%', '0.27%'],
        answer: 2,
        explanation:
          'A task reaches the dead-letter queue only if all three attempts fail: 0.3 x 0.3 x 0.3 = 0.027, about 2.7%. 30% is the chance of one failed attempt, 9% the chance that two fail, and 0.27% slips a decimal place. The Dead-lettered metric in the Lab settles near this share.',
      },
      {
        id: 'tq-4',
        prompt:
        ...
        options: [
          'Only the dead-letter queue grows, since failed tasks leave after 3 attempts',
          'Nothing - 40 tasks/sec is still well under the 60/sec the workers can do, retries included',
          'The workers speed up to handle the retries',
          'Retries raise the load to about 78 attempts/sec, above 60, so the queue grows too',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'About 400 extra calls per second in synchronised waves; add backoff with jitter',
          'No extra load - a retry policy waits until the API reports healthy before it sends anything',
          'About 5 extra calls per second, because retries are spaced 5 s apart',
          'The 2,000 calls arrive once, spread evenly over the 5 s delay, then stop',
        ],
        answer: 0,
        explanation:
      ...
      },
      {
        ...
        options: [
          'Nothing - the task has all the data it needs, so it avoids a database read',
          'Tasks must always run immediately after enqueue, so the object never has time to go stale',
          'Raise the message size limit',
          'It works on stale data; pass the order id and load the order when the task runs',
        ],
        answer: 3,
        explanation:
      ...
      },
      {
        ...
        options: [
          'A delayed task with run_at set six months ahead, so the broker holds it until then',
      ...
      },
      {
        ...
        options: [
          ...
          'The reports should be made high priority too, so they get their turn',
          'One queue with priorities starves low-priority work; use separate queues',
          'The queue needs a bigger maximum size, so reports stop being pushed out',
        ],
        answer: 2,
        explanation:
      ...
      },
      {
        ...
        options: [
          'The dead-letter queue is too small to hold a week of failures',
          'The retry count is too low, so tasks dead-letter too early',
          'Nothing - the tasks are safe in the dead-letter queue',
          'Nobody watches it: that is data loss with a delay; alert on its depth',
        ],
        answer: 3,
        explanation:
          'The dead-letter queue exists so a person can look at failures. Without an alert, 3,000 customers have not been invoiced and nobody knows. Alert on its depth, fix the cause, then redrive the tasks. More retries would not have fixed tasks that failed for a real reason.',
      },
      {
        id: 'tq-10',
        ...
        options: [
          'Nothing - the broker kills a task that runs past its visibility timeout',
          'Each hang holds a worker for good; set a per-task timeout',
          'The visibility timeout ends the hung call and frees the worker',
          'The queue notices the task is slow and skips to the next one',
        ],
        answer: 1,
        explanation:
