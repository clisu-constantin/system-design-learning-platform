...
cost                 very low          CPU for TLS, a little latency`,
      ...
      },
      {
        ...
        paragraphs: [
          ...
          'Least connections sends the next request to the backend with the fewest open connections, which naturally compensates for uneven request cost, at the price of tracking every connection. It has a blind spot of its own: a crashed backend that refuses connections instantly holds zero of them, so until a health check removes it, it wins almost every pick. Least response time goes further by preferring the fastest backend, but it can herd traffic onto a node that only looks fast because it is returning errors quickly.',
        ...
        bullets: [
          ...
          'Least connections: adapts when request cost varies; needs health checks to avoid dead servers.',
      ...
      },
      {
        ...
        paragraphs: [
          ...
          'Shallow checks (a TCP connect, or a 200 from /ping) prove the process is alive but not that it can work. Deep checks that verify the database connection catch more real failures - and can also take your entire fleet out at once when the database has a brief hiccup, because every instance fails the check simultaneously. The usual compromise: the check the balancer uses covers what this one instance needs to serve, not every shared dependency, and many balancers fail open - an AWS Application Load Balancer sends traffic to every target when all of them fail their checks - so one blip cannot empty the whole pool.',
          'Two settings matter as much as the check itself. Detection is not instant: HAProxy probes every 2 s by default and marks a server down after 3 failures in a row, so a crashed server keeps receiving requests for up to about 6 s. Tune the threshold so a single slow response does not eject a healthy node, and make sure the balancer drains connections before removing a node during a deploy - that is the difference between a rolling deploy and a rolling error spike.',
        ],
      },
    ],
    examples: [
    ...
    remember: [
      ...
      'Round robin ignores how busy a server is; least connections adapts to uneven request cost, at the price of tracking connections.',
